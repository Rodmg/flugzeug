...
export class AuthController extends Controller {
  ...
  private sendEmailNewPassword(user: any, token: string, name?: string): Promise<any> {
    ...
    return mailer.sendEmail(user.email, subject, 'password_recovery', user.profile.locale, {
  ...
  private sendEmailPasswordChanged(user: any, name?: string): Promise<any> {
    ...
    return mailer.sendEmail(user.email, subject, 'password_changed', user.profile.locale, {
      name : name || user.email,
    })
    .then(info => {
  ...
  private handleResetChPass(token: string, password: string): Promise<any> {
    return this.validateJWT(token, 'reset')
      .then((decodedjwt) => {
        ...
        return User.findOne({ where: { id: decodedjwt.id }, include: [ { model: Profile, as: 'profile' } ] })
          .then((user) => {
            if(!user) {
  ...
  */
  ...
  changePassword(req: Request, res: Response) {
    ...
    if(email.length === 0 || oldPass.length === 0 || newPass.length === 0) return Controller.badRequest(res);
