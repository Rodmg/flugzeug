...
export function setupSockets(): Promise<any> {
  ...
  io.on('connection', (socket) => {
    ...
    log.info("Websocket connected, client:", socket.handshake.address, socket.request.session.user.email);
    ...
    socket.on('disconnect', (reason) => {
      log.info("Websocket disconnected, client:", socket.handshake.address, socket.request.session.user.email, "reason:", reason);
